...
% Script to Input "Magic Number" Variables and Data:
    ...
    % Load in Sock:
        ...
        Geometry = load('/Users/lindsayrupp/Documents/CEG Research/Pacing Experiment Data/14-10-27/Geometry Files/Registered Cartesian Sock.mat'); % NOTE: CHANGE THIS LOCATION DEPENDING ON WHERE FILE IS STORED!!!!!!!!!!
                ...
                for Third_Index = 1:Number_of_Beats_per_Run
                    ...
                    % From File Obtain the Fiducials:
                        ...
                        % QRS On:
                            ...
                            QRS_Fiducial = find(Fiducial_Types == 2); % 2 is QRS on 
                            QRS_Time = Fiducial_Structure(QRS_Fiducial);
                            QRS_Time = getfield(QRS_Time, 'value'); % Get QRS Values

                        % Activation Time:

                            Activation_Fiducial = find(Fiducial_Types == 10); % 10 is Activation Times 
                            Activation_Time = Fiducial_Structure(Activation_Fiducial);
                            Activation_Time = getfield(Activation_Time, 'value'); % Get Activation Values

                        % Overall Activation Time:

                            Overall_Activation_Times = Activation_Time - QRS_Time; % Want to Find Electrodes with the Earliest/Smallest Activation Times
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    % Script to Interpolate Around Bad Leads:
                        ...
                        % Laplacian Interpolation:
                            ...
                            Bad_Leads = load('/Users/lindsayrupp/Documents/CEG Research/Pacing Experiment Data/14-10-27/Bad_Leads_14_10_27.mat'); % NOTE: CHANGE THIS LOCATION DEPENDING ON WHERE FILE IS STORED!!!!!!!!!!
                                Bad_Leads = Bad_Leads.Bad_Leads_14_10_27; % NOTE: CHANGE THIS LOCATION DEPENDING ON WHERE VARIABLE IS STORED!!!!!!!!!!
                                    Bad_Lead_Value = find(Bad_Leads == 0);
                        ...
                        % Create Vector Containing Activation Times for Only Good Leads:
                            ...
                            Good_Lead_Activation_Times_Locations = find(Zeroed_Activation_Times ~= 0);
                            Good_Lead_Activation_Times = Overall_Activation_Times(Good_Lead_Activation_Times_Locations);

                        % Apply Mapping Matrix Obtained from SCIRun:

                            % Note: Format: Mapping_Matrix (All X Good) * Good Leads (Good X 1) = Interpolated (All X 1).

                                Interpolated_Activation_Times = Mapping_Matrix * Good_Lead_Activation_Times;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    for Fourth_Index = (ceil(min(Upsampled_Interpolated_Activation_Times)) + 1):(floor(max(Upsampled_Interpolated_Activation_Times)) - 1)
                    ...
                    % Script to Determine Coordinates in the Breakthrough Site:
                        ...
                        [Activated_Electrode_X_Location, Activated_Electrode_Y_Location, Activated_Electrode_Z_Location] = Time_Breakthrough_Site_Electrode_Determination_Function(Centered_Upsampled_Points, Upsampled_Interpolated_Activation_Times, Fourth_Index);

                        if size(Activated_Electrode_X_Location, 1) == 0

                            Ellipse_Information.Area.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = NaN;
                            Ellipse_Information.Axis_Ratio.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = NaN;
                            Ellipse_Information.Angle_for_Long_Axis.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = NaN;
                            Ellipse_Information.Angle_for_Short_Axis.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = NaN;
                            ...
                            % Check to See if an Ellipse was Fitted in Order to Decide Whether or Not Parameters can be Calculated:
                                ...
                                if size(Ellipse_Structure.a, 1) == 0
                                    ...
                                    [Temporary_Ellipse_Information] = Calculate_Ellipse_Parameters_Function(Ellipse_Structure, Horizontal_Line, Vertical_Line);
                                        ...
                                        Ellipse_Information.Area.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = Temporary_Ellipse_Information.Area;
                                        Ellipse_Information.Axis_Ratio.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = Temporary_Ellipse_Information.Axis_Ratio;
                                        Ellipse_Information.Angle_for_Long_Axis.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = Temporary_Ellipse_Information.Angle_for_Long_Axis;
                                        Ellipse_Information.Angle_for_Short_Axis.Time(Fourth_Index, 1).Run(First_Index, 1).Beat(Third_Index, 1) = Temporary_Ellipse_Information.Angle_for_Short_Axis;
