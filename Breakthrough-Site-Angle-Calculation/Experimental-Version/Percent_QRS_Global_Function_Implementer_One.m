...
% Script to Input "Magic Number" Variables and Data:
    ...
    % Load in Time Signal:
        ...
        Run_Number = 37; % NOTE: CHANGE THIS DEPENDING ON WHAT RUN I WANT TO LOOK AT!!!!!!!!!!!
        Beat_Number = 6; % NOTE: CHANGE THIS DEPENDING ON WHAT BEAT I WANT TO LOOK AT!!!!!!!!!!!
        ...
        % Grab File:
            ...
            % From File Obtain the Fiducials:
                ...
                % Activation Time:
                    ...
                    Activation_Times = Fiducial_Structure(Activation_Fiducial);
                    Activation_Times = getfield(Activation_Times, 'value'); % Get Activation Values
...
% Script to Interpolate Around Bad Leads:
    ...
    % Create Vector Containing Activation Times for Only Good Leads:
        ...
        Zeroed_Activation_Times = Activation_Times;
        Zeroed_Activation_Times(Bad_Lead_Value) = 0;
        
        Good_Lead_Activation_Times_Locations = find(Zeroed_Activation_Times ~= 0);
        Good_Lead_Activation_Times = Activation_Times(Good_Lead_Activation_Times_Locations);
...
% Script to Determine Coordinates in the Breakthrough Site:
    ...
    Time_Points_of_Interest = linspace(QRS_On_Time, QRS_Off_Time, 100);

    Time_Into_QRS_Peak = Time_Points_of_Interest(Percent_into_QRS_Peak);

    % Functions: Determine Which Electrodes Have a Activation Time Less than the Set QRS Time.

    [Activated_Electrode_X_Location, Activated_Electrode_Y_Location, Activated_Electrode_Z_Location] = Time_Breakthrough_Site_Electrode_Determination_Function(Centered_Upsampled_Points, Upsampled_Interpolated_Activation_Times, Time_Into_QRS_Peak);
    
    if size(Activated_Electrode_X_Location, 1) < 2
